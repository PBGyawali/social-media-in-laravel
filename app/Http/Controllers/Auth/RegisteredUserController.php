...
class RegisteredUserController extends Controller
{
    ...
    {
        $request->validate([
            'registry_username' => 'required|max:255|unique:users,username|different:registry_email',
            'registry_email' => 'required|email|max:255|unique:users,email|confirmed|different:password',
            'password' => ['required','different:registry_username','confirmed', Rules\Password::defaults()],
        ]);
        $email=$request->registry_email;
