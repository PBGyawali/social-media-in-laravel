...
use Illuminate\Support\Str;
	class Select {
        ...
        private function list($category,$value,$column=null,$status='active',$category_id=null,$attr=array())
        {
            ...
            $query = DB::table(Str::plural($category))->orderBy($columnname?$columnname:$category."_name", 'ASC');
            if($placeholder)
            foreach($placeholder as $key=> $columnattribute)
            {
              $query->where($columnattribute, $compare, $condition[$key]);
            }
            ...
            foreach($result as $row)
            {
                ...
                $output .= '<option value="'.$row->{$columnname?$columnname:$category."_id"}.'"'. $selected.' '.$disabled.  '>'.
                $row->{$column?$category."_name":($columnname?$columnname:$category."_name")}.
                   ' '.(($column=='percentage')?"(".$row->{$category."_".$column}:"").$extratext.'</option>';
            }
            return $output;
        }
        ...
        {
            $currency_data = Self::currency_array();
        ...
        function short_country_list($value=null)
        {
            $country = array(
                'AF' => 'Afghanistan',
                'AX' => 'Aland Islands',
            ...
            $html = '<select name="owner_country" id="owner_country" class="form-control" required>
                <option value="">Select Country</option>	';
                $selected_value = $value;// value from database
            foreach($country as $keys => $values)
            {
                $selected = ($selected_value == $keys) ? "selected" : "";
                $html .= '<option value="'.$keys.'"'. $selected.'>'.$values.'</option>';
            }
            $html .= '</select>';
            return $html;
        }

        function Country_list($value=null)
	{
	$country=array(
    ...
    static function Currency_list($value=null)
	{
		$html = '<select name="website_currency" id="website_currency" class="form-control" required>
			<option value="">Select Currency</option>';
		$datas =  Self::currency_array();
		$selected_value = $value;// value from database
		foreach($datas as $data)
		{	$selected = ($selected_value == $data["code"]) ? "selected" : "";
			$html .= '<option value="'.$data["code"].'"'. $selected.'>'.$data["name"].'</option>';
		}
		$html .= '</select>';
		return $html;
    }

    static function Timezone_list($value=null)
	{
		$timezones = Self::Timezone_array();
		$html = '<select name="website_timezone" id="website_timezone" class="form-control" required>
    ...
    static function currency_array()
	{
		$currencies = array(
			array('code'=> 'ALL',
			'countryname'=> 'Albania',
    ...
    static function Timezone_array()
	{
		$timezones = array(
			...
			'Asia/Kathmandu' => '(GMT+5:45) Asia/Kathmandu (Nepal Time)',
