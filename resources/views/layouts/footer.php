...
if($('#table').length>0){
  ...
  $(document).on('click', 'button.status', function(){
    ...
    $('#report').click(function(){
      ...
      $('#export').click(function(){
  		...
  		if(from_date != '' && to_date != '')
  			window.location.href=url+"export/from_date="+from_date+"&to_date="+to_date+"";
  		else
    ...
    function Post_update(data){
        var publish_status=data.response.publish_status;
        $('#publish,#anonymity').attr('checked',false);
        if (['1','active'].includes(publish_status)) $('#publish').attr('checked',true);
