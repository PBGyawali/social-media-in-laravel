<div class="container" id="register_container">
  ...
  <div class="form-input-group">
    <label class="d-inline-block">Username</label><span class="d-inline-block position-relative " style="z-index:978"> </span>
  	  <input type="text" name="username" id="register_username" value="<?= old('username')?>"class="form-control datacheck" data-object="Username"  required data-parsley-minlength="5" data-parsley-trigger="on change">
        <?= $errors->has('username')? '<span class="text-danger">'.$errors->first('username').'</span>':''?>

    </div>
  	<div class="form-input-group">
  	  <label class="d-inline-block">Email</label> <span class="d-inline-block position-relative "></span>
  	  <input type="email" name="registry_email" value="<?= old('registry_email')?>"id="register_email" class="form-control datacheck" data-object="Email"  required data-parsley-type="email" data-parsley-trigger="on change">
        <?=  $errors->has('registry_email')? '<span class="text-danger">'.$errors->first('registry_email').'</span>':''?>

    </div>
	  <div class="form-input-group">
		<label>Confirm Email</label>
		<span></span>
  	  <input type="email" name="registry_email_confirmation" value="<?= old('registry_email_confirmation')?>" id="register_confirmemail" class="form-control"required data-parsley-equalto="#register_email" data-parsley-trigger="on change">
        <?=  $errors->has('registry_email_confirmation')? '<span class="text-danger">'.$errors->first('registry_email_confirmation').'</span>':''?>
    </div>
  	<div class="form-input-group">
