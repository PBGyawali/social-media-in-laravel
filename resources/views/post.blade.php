				...
				<?php if (auth()->user()->is_editor()): ?>
					<!-- display checkbox according to whether post has been published or not -->
						<label class="col-md-1 text-left">Publish<?=(isset($post) && $post->is_published())?'ed':'' ?></label>
							...
							<input type="checkbox" name="publish_status" id="publish" value="active" <?=old('publish_status')=='active'?'checked="checked"':((isset($post) && $post->is_published())?'checked="checked"':'' )?> class="form-control" >
                            <?php if ($errors->has('publish_status')) echo '<span class="text-danger">'.$errors->first('publish_status').'</span>';?>
                        </div>
				<?php endif ?>
