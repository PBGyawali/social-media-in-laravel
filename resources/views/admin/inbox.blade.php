                            ...
                            <div class="email-content">
								<div class="table-responsive">
									<table class="table table-inbox table-hover">
										...
										<tbody>
                                            @foreach ($emails as $email)                                         
                                                <tr class="{{ ['','unread font-weight-bold'][array_rand([1,2])] }} clickable-row" href="{{route('support.show',['id'=>$email->getKey()])}}">
                                                    <td>
                                                        <input type="checkbox" class="checkmail ">
                                                    </td>
                                                    <td><span class="mail-important"><i class="fa-star {{ ['far','fas text-warning'][array_rand([1,2])] }}"></i></span></td>
                                                    <td class="name">{{[$email->full_name,'me,'.($names[array_rand($names)]).'('.rand(1, 10).')'][array_rand([1,2])]}}</td>
                                                    <td class="subject">{{$email->subject}}</td>
                                                    <td>{!! ['','<i class="fa fa-paperclip"></i>'][array_rand([1,2])] !!}</td>
                                                    <td class="mail-date">{{$email->created_at}}</td>                                                  
                                                </tr>                                       
                                            @endforeach	                                                                                        																
										</tbody>
									</table>
                                    ...
                                    <div class="table table-inbox table-hover">										
                                            ...
                                            <div class="row pl-3 border border-light py-2 {{ ['','unread font-weight-bold'][array_rand([1,2])] }} clickable-row" href="{{route('support.show',['id'=>$email->getKey()])}}">
                                            <div class="col-1">
                                                <input type="checkbox" class="checkmail ">
                                            </div>
                                            <div class="col-1">
                                                <span class="mail-important"><i class="fa-star {{ ['far','fas text-warning'][array_rand([1,2])] }}"></i></span>
                                            </div>
                                            
                                            <div class="col-3 name">
                                                {{[$email->full_name,'me,'.($names[array_rand($names)]).'('.rand(1, 10).')'][array_rand([1,2])]}}
                                            </div>
